...
class DRAGGEnv(gym.Env):
    ...
    def get_reward(self, obs):
        ...
        reward = -1*(sp - self.agg.agg_load)**2 - self.agg.lam*np.clip((self.agg.max_load - (2.5 * self.agg.config['community']['total_number_homes'][0])),0,None)
        print("REWARD BEFORE NORM", reward)
        reward = (reward - self.agg.n_avg_reward) / (self.agg.n_max_reward - self.agg.n_min_reward)
        print("REWARD AFTER NORM", reward)
